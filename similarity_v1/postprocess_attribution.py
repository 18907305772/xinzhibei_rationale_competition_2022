...
def prepare_eval_data(data, results, RATIONALE_RATIO=0.705):
    res = {}
    for d, inter_res in zip(data, results):
        # Split importance score vectors for query and title from inter_res.word_attributions
        query_importance_score = np.array(inter_res.word_attributions[1:len(d['text_q_seg'])+1])
        title_importance_score = np.array(inter_res.word_attributions[len(d['text_q_seg'])+2:-1])
        # Extract topK importance scores
        query_topk = math.ceil(len(d['text_q_seg'])*RATIONALE_RATIO)
        title_topk = math.ceil(len(d['text_t_seg'])*RATIONALE_RATIO)
